...
Warum gibt es hier einen pandas-Patch?
bat_model_v01_fast.py legt in manchen Funktionen pandas.Series mit int64 dtype an.
Neuere pandas-Versionen erlauben dann nicht mehr, Float-Werte wie 3.93 einzutragen.
Der Patch sorgt dafür, dass diese internen Series als float64 erzeugt werden.
"""
...
# ---------------------------------------------------------------------------
# Compatibility patch for newer pandas versions
# ---------------------------------------------------------------------------

def _patch_bat_model_for_pandas_float_dtype() -> None:
    """
    Patcht init_step und init_step_rest aus bat_model_v01_fast.py so,
    dass die erzeugten Series float64 sind.

    Das behebt Fehler wie:
    TypeError: Invalid value '3.9299999999999997' for dtype 'int64'
    """
...
def vehicle_power_to_cell_power_w(vehicle_power_kw: float, battery_capacity_kwh: float) -> float:
    ...
    Fahrzeugleistung muss deshalb auf Zellleistung skaliert werden.
    """
